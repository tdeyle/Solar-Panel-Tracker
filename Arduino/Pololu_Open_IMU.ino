...
Based on the Madgwick algorithm found at:
 ...
 Using the repeater program, this device shifts out 6 bytes of data to the prop for processing.
 ...
 */
/*M min X: -342 Y: -406 Z: -321 M max X: 287 Y: 353 Z: 336

*/

#define compassXMax 287.0f
#define compassXMin -342.0f
#define compassYMax 353.0f
#define compassYMin -406.0f
#define compassZMax 336.0f
...
void loop(){
  ...
  if (micros() - timer >= 5000){
    ...
    ipitch[1] = (byte) ((int) pitch);
    ipitch[2] = (byte) ((int) pitch >> 8);
    
    //temp = roll;
    if (roll < 0.0){
      iroll[0] = 1;
    }
    iroll[1] = (byte) ((int) roll);
    iroll[2] = (byte) ((int) roll >> 8);
    ...
    iyaw[1] = (byte) ((int) yaw);
    iyaw[2] = (byte) ((int) yaw >> 8);
    
    
    Serial3.write("!");
    Serial3.write(ipitch, 3);
    Serial3.write(iroll, 3);
    Serial3.write(iyaw, 3);
   
    //Wire.write(iroll);
    //Wire.write(iyaw);
        
    /*Serial3.write("!");
    Serial3.println(ipitch);
    Serial3.println(iroll);
    Serial3.println(iyaw);
    */
    
    
    
    /*Serial3.write((byte) ipitch);
    Serial3.write((byte) (ipitch >> 8));
    Serial3.write((byte) iroll);
    Serial3.write((byte) (iroll >> 8));
    Serial3.write((byte) iyaw);
    Serial3.write((byte) (iyaw >> 8));   
    */
  }
...
void IMUinit(){
  ...
  gyro.writeReg(L3G_CTRL_REG1, 0xCF); //$20
  gyro.writeReg(L3G_CTRL_REG2, 0x00); //$21
  gyro.writeReg(L3G_CTRL_REG3, 0x00); //$22
  gyro.writeReg(L3G_CTRL_REG4, 0x20); //$23
  gyro.writeReg(L3G_CTRL_REG5, 0x02); //$24

  compass.writeAccReg(LSM303_CTRL_REG1_A, 0x77);//400hz all enabled $20
  compass.writeAccReg(LSM303_CTRL_REG4_A, 0x20);//+/-8g 4mg/LSB     $23

  compass.writeMagReg(LSM303_CRA_REG_M, 0x1C); //$00
  compass.writeMagReg(LSM303_CRB_REG_M, 0x60); //$01
  compass.writeMagReg(LSM303_MR_REG_M, 0x00);  //$02
