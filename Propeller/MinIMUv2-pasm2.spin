...
VAR
  ...
  long addAccl, addGyro, addMag

 
PUB Start( _SCL, _SDA) : Status
'PUB Start( _SCL, _SDA, AcclAddress, GyroAddress, MagAddress) : Status
...
PUB readAccl(acclAddress)
'Takes a reading from the Accelerometer and stores it at the accAddress
  long[acclAddress][0] := GetAX
  long[acclAddress][1] := GetAY
  long[acclAddress][2] := GetAZ      
                          
PUB readGyro(gyroAddress)
'Takes a reading from the Gyro and stores it at the gyroAddress
  long[gyroAddress][0] := GetRX
  long[gyroAddress][1] := GetRY
  long[gyroAddress][2] := GetRZ  

PUB readMag(magAddress)
'Takes a reading from the Accelerometer and stores it at the magAddress
  long[magAddress][0] := GetMX
  long[magAddress][1] := GetMY
  long[magAddress][2] := GetMZ  
...
:GyroSetConfig          mov     i2cDevID, #%11010110     'Device ID for the MinIMU gyro (was L3G4200D now L3GD20)
                                                         'DEFAULT Settings:
                                                         '0x20 -> 400Hz, 110 cutoff, Normal power mode, all axes enabled
                                                         '0x23 -> 250dps, Continuous Enabled

                        mov     i2cAddr, #$20            'Config_reg 1
                        'mov     i2cValue, #%10_00_1111   '400Hz, 110 cutoff, Normal power mode, all axes enabled -> Here
                        mov    i2cValue, #%11_00_1111   '760Hz, 30 cutoff, Normal power mode, all axes enabled -> OpenIMU
                                                          
                        call    #i2cWriteRegisterByte

                        mov    i2cAddr, #$21            'Config Reg 2
                        mov    i2cValue, #%00_00_0000   'Normal Mode (Reset) -> Init from Pololu OpenIMU from Mikeshub

                        call    #i2cWriteRegisterByte

                        mov    i2cAddr, #$22            'Config Reg 3
                        mov    i2cValue, #%0_0_00_000_0 ' -> Init from Pololu OpenIMU from Mikeshub

                        call    #i2cWriteRegisterByte
                        
                        mov     i2cAddr, #$23            'Config Reg 4
                        mov     i2cValue, #%0_0_10_000_0 '2000 deg/sec -> Init from Pololu OpenIMU from Mikeshub and here
                        
                        call    #i2cWriteRegisterByte

                        mov    i2cAddr, #$24            'Config Reg 5
                        mov    i2cValue, #%0_0_0_0_0010 'Wait Enabled -> Init from Pololu OpenIMU from Mikeshub and here

                        call    #i2cWriteRegisterByte
                                                                                                    
:AccelSetConfig         mov     i2cDevID, #%00110010     'Device ID for the MinIMu accelerometer (LSM303)
                                                         'DEFAULT Settings:
                                                         '0x20 -> Normal power, 400hz data, all axis enabled,
                                                         '0x23 -> +/-2g, HR Disabled, Continuous Enabled

                        mov     i2cAddr, #$20            'CTRL_REG1_A
                        mov     i2cValue, #%0111_0_111   'Normal power, 400hz data, all axis enabled,-> Init from Pololu OpenIMU from Mikeshub and here.
                        'mov    i2cValue, #%0100_0_111   'Normal power, 50Hz data, all axis enabled, -> Init from LSM303.cpp

                        call    #i2cWriteRegisterByte

                        mov     i2cAddr, #$23            'CTRL_REG4_A
                        mov     i2cValue, #%0010_0000    '+/- 8g, HR Disabled, Continuous Enabled -> Init from Pololu OpenIMU from Mikeshub
                        'mov    i2cValue, #%0000_1000    '+/- 2g, HR Enabled, Continuous Enabled -> Init from LSM303.cpp
                        'mov    i2cValue, #%0010_1000    '+/- 8g, HR Enabled, Continuous Enabled
...
:MagSetConfig           mov     i2cDevID, #%00111100     'Device ID for the MinIMu magnetometer (LSM303)
                                                         'DEFAULT Settings:
                                                         '0x00 -> 15Hz ODR, Temp Sensor Disabled
                                                         '0x01 -> 1.3Gauss
                                                         '0x02 -> Continuous Conversion
                                                         
                        mov     i2cAddr, #$00            'CRA_REG_M
                        'mov     i2cValue, #%000_110_00   '75Hz magnetometer updates ->  Here
                        mov    i2cValue, #%000_111_00   '220Hz magnetometer updates -> Pololu OpenIMU from Mikeshub
                        'mov    i2cValue, #%000_011_00   '7.5Hz magnetometer updates -> Init from LSM303.cpp

                        call    #i2cWriteRegisterByte

                        mov     i2cAddr, #$01            'CRB_REG_M
                        'mov     i2cValue, #%001_00000    '1.3 Gauss field input (high gain setting) -> Here and LSM303.cpp
                        mov    i2cValue, #%011_00000    '2.5 Gauss field input (high gain setting) -> Pololu OpenIMU from Mikeshub

                        call    #i2cWriteRegisterByte

                        mov     i2cAddr, #$02            'MR_REG_M
                        mov     i2cValue, #%000000_00    'Continuous conversion mode -> Everywhere
...
loopDelay               long    80_000_000 / 400        '2x oversampling @ 400hz = 200 hz was 80_000_000
loopCount               long    0
